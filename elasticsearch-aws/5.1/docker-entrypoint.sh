...
set -- "$@" -E network.publish_host=${AWS_PRIVATE_IP} \
            -E discovery.type=ec2 \
            -E discovery.ec2.groups=${AWS_SECURITY_GROUP_IDS}

echo "Elasticsearch Publish Host: ${AWS_PRIVATE_IP}"
echo "Security Group: ${AWS_SECURITY_GROUP_IDS}"

# Requires a privileged docker container (--privileged) to execute. Keeps elasticsearch bootstrap checks happy
